        ...
        <Dialog.Root>
          ...
          <Dialog.Portal>
            <Dialog.Overlay className='inset-0 fixed bg-black/50'/>
              <Dialog.Content className='overflow-hidden fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2  max-w-[640px] h-[68vh] w-full bg-slate-700 rounded-md flex flex-col outline-none'>
                <Dialog.Close 
                onClick={handleClose}
                ...
                <form onSubmit={handleSaveNote} className='flex-1 flex flex-col'>
                  ...
                  {shouldShowOnboarding ? (
                    <p>Comece <button className='font-medium text-lime-400 hover:underline'>gravando uma nota</button> em aúdio ou se preferir <button onClick={handleStartEditor} className='font-medium text-lime-400 hover:underline'>utilize apenas texto.</button></p> 
                  ):(
                    <textarea 
                    autoFocus
